...
for adframe_idx = 1:numads
    curr_clusTrackIDs = clusTrackIDs{adframe_idx};
    initial_nclus = numel(curr_clusTrackIDs);

    original_RI_distribution = cell(numel(curr_clusTrackIDs),1);

    %% Reassign tracks to clusters!
    for j=1:initial_nclus
        trackIDs = curr_clusTrackIDs{j};
        track_starts = cellfun(@(x) x.startFrame,expdata.corrDataStruct(trackIDs));
        ...
        if any(all_recruitment_intervals>RIcutoff)
            ...
            for k=1:nsubclusters
                ...
                cluster_track_IDs{adframe_idx} = [cluster_track_IDs{adframe_idx},trackStartandID(subcluster_idxs,2)'];
            end
        else % the whole cluster was 'normal', we don't even need to use re-sorted information.
            cluster_track_IDs{adframe_idx} = [cluster_track_IDs{adframe_idx},{trackIDs}];
        end
    end
    ...
    savename = [parentFigDir figName sprintf('_adframe%02i',adframe_idx)];
