...
density_distr = cell(numads,1);


for adframe_idx = 1:numads % Each adhesion frame.
    current_tracks = cellfun(@(x) x.adhesionFrame==adframe_idx,expdata.corrDataStruct);
    num_tracks = sum(current_tracks);
    density_distr{adframe_idx} = cell(nsims,1);
    ...
    for sim_rep_idx = 1:nsims
        ...
        density_distr{adframe_idx}{sim_rep_idx} = xyden(:,3);
    end % sim reps
end
end
