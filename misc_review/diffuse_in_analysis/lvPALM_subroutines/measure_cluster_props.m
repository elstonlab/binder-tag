...
% Load variational Bayes SPT data. This contains diffusional dynamics analyses.
...
function properties = calculate_cluster_properties(clusterTrackIDs,expdatafile,ROIfile,vbSPTfile,vbSPTlinkagefile)
%% Progress update
...
% Figure out what the last observed frame will be.
SPTacqFrame = max(cellfun(@(x) x(end,end),expdata.smLinked));
...
n_clus = numel(clusterTrackIDs);

% Setting up the list of properties to measure in this function.
% Many of them are not used in the manuscript but were explored at some point.
...
% For diffuse-in analysis
properties.allLoc.KDEeffR50_removeDiffuseIn = zeros(n_clus,1); % effective radius from all regions at 50% max via kernel density estimation
properties.allLoc.KDEeffR95_removeDiffuseIn = zeros(n_clus,1); % effective radius from all regions at 5% max via kernel density estimation
properties.allLoc.KDEcirc50_removeDiffuseIn = zeros(n_clus,1); % circularity from all regions at 50% max via kernel density estimation
properties.allLoc.KDEcirc95_removeDiffuseIn = zeros(n_clus,1); % circularity from all regions at 5% max via kernel density estimation
properties.allLoc.KDEeccen50_removeDiffuseIn = zeros(n_clus,1); % eccentricity from all regions at 50% max via kernel density estimation
properties.allLoc.KDEeccen95_removeDiffuseIn = zeros(n_clus,1); % eccentricity from all regions at 5% max via kernel density estimation
properties.allLoc.KDEarea50_removeDiffuseIn = zeros(n_clus,1); % area from all regions at 50% max via kernel density estimation
properties.allLoc.KDEarea95_removeDiffuseIn = zeros(n_clus,1); % area from all regions at 5% max via kernel density estimation
properties.allLoc.KDEmajAx50_removeDiffuseIn = zeros(n_clus,1); % majoraxis from all regions at 50% max via kernel density estimation
properties.allLoc.KDEmajAx95_removeDiffuseIn = zeros(n_clus,1); % majoraxis from all regions at 5% max via kernel density estimation
properties.allLoc.KDEminAx50_removeDiffuseIn = zeros(n_clus,1); % minoraxis from all regions at 50% max via kernel density estimation
properties.allLoc.KDEminAx95_removeDiffuseIn = zeros(n_clus,1); % minoraxis from all regions at 5% max via kernel density estimation
properties.allLoc.KDEbound50_removeDiffuseIn = cell(n_clus,1); % boundaries from 50% max via kernel density estimation
properties.allLoc.KDEbound95_removeDiffuseIn = cell(n_clus,1); % boundaries from 5% max via kernel density estimation
...
properties.allLoc.COM_removeDiffuseIn = zeros(n_clus,2); % all localization-based center of mass
properties.allLoc.KDEmaskCOM50_removeDiffuseIn = zeros(n_clus,2); % center of mass of the 50%KDE threshold mask
properties.allLoc.KDEmaskCOM95_removeDiffuseIn = zeros(n_clus,2); % center of mass of the 95%KDE threshold mask
...
properties.vbSPT.sloweffR50 = zeros(n_clus,1); % effective radius from all regions at 50% max via kernel density estimation
properties.vbSPT.sloweffR95 = zeros(n_clus,1); % effective radius from all regions at 5% max via kernel density estimation
properties.vbSPT.fasteffR50 = zeros(n_clus,1); % effective radius from all regions at 50% max via kernel density estimation
properties.vbSPT.fasteffR95 = zeros(n_clus,1); % effective radius from all regions at 5% max via kernel density estimation
properties.vbSPT.slowcirc50 = zeros(n_clus,1); % circularity from all regions at 50% max via kernel density estimation
properties.vbSPT.slowcirc95 = zeros(n_clus,1); % circularity from all regions at 5% max via kernel density estimation
properties.vbSPT.fastcirc50 = zeros(n_clus,1); % circularity from all regions at 50% max via kernel density estimation
properties.vbSPT.fastcirc95 = zeros(n_clus,1); % circularity from all regions at 5% max via kernel density estimation
properties.vbSPT.sloweccen50 = zeros(n_clus,1); % eccentricity from all regions at 50% max via kernel density estimation
properties.vbSPT.sloweccen95 = zeros(n_clus,1); % eccentricity from all regions at 5% max via kernel density estimation
properties.vbSPT.fasteccen50 = zeros(n_clus,1); % eccentricity from all regions at 50% max via kernel density estimation
properties.vbSPT.fasteccen95 = zeros(n_clus,1); % eccentricity from all regions at 5% max via kernel density estimation
...
properties.vbSPT.sloweffR50_removeDiffuseIn = zeros(n_clus,1); % effective radius from all regions at 50% max via kernel density estimation
properties.vbSPT.sloweffR95_removeDiffuseIn = zeros(n_clus,1); % effective radius from all regions at 5% max via kernel density estimation
properties.vbSPT.fasteffR50_removeDiffuseIn = zeros(n_clus,1); % effective radius from all regions at 50% max via kernel density estimation
properties.vbSPT.fasteffR95_removeDiffuseIn = zeros(n_clus,1); % effective radius from all regions at 5% max via kernel density estimation
properties.vbSPT.slowcirc50_removeDiffuseIn = zeros(n_clus,1); % circularity from all regions at 50% max via kernel density estimation
properties.vbSPT.slowcirc95_removeDiffuseIn = zeros(n_clus,1); % circularity from all regions at 5% max via kernel density estimation
properties.vbSPT.fastcirc50_removeDiffuseIn = zeros(n_clus,1); % circularity from all regions at 50% max via kernel density estimation
properties.vbSPT.fastcirc95_removeDiffuseIn = zeros(n_clus,1); % circularity from all regions at 5% max via kernel density estimation
properties.vbSPT.sloweccen50_removeDiffuseIn = zeros(n_clus,1); % eccentricity from all regions at 50% max via kernel density estimation
properties.vbSPT.sloweccen95_removeDiffuseIn = zeros(n_clus,1); % eccentricity from all regions at 5% max via kernel density estimation
properties.vbSPT.fasteccen50_removeDiffuseIn = zeros(n_clus,1); % eccentricity from all regions at 50% max via kernel density estimation
properties.vbSPT.fasteccen95_removeDiffuseIn = zeros(n_clus,1); % eccentricity from all regions at 5% max via kernel density estimation

properties.vbSPT.slowarea50_removeDiffuseIn = zeros(n_clus,1); % area at 50% max via kernel density estimation
properties.vbSPT.slowarea95_removeDiffuseIn = zeros(n_clus,1); % area at 5% max via kernel density estimation
properties.vbSPT.fastarea50_removeDiffuseIn = zeros(n_clus,1); % area at 50% max via kernel density estimation
properties.vbSPT.fastarea95_removeDiffuseIn = zeros(n_clus,1); % area at 5% max via kernel density estimation
properties.vbSPT.slowmajAx50_removeDiffuseIn = zeros(n_clus,1); % majoraxis at 50% max via kernel density estimation
properties.vbSPT.slowmajAx95_removeDiffuseIn = zeros(n_clus,1); % majoraxis at 5% max via kernel density estimation
properties.vbSPT.fastmajAx50_removeDiffuseIn = zeros(n_clus,1); % majoraxis at 50% max via kernel density estimation
properties.vbSPT.fastmajAx95_removeDiffuseIn = zeros(n_clus,1); % majoraxis at 5% max via kernel density estimation
properties.vbSPT.slowminAx50_removeDiffuseIn = zeros(n_clus,1); % minoraxis at 50% max via kernel density estimation
properties.vbSPT.slowminAx95_removeDiffuseIn = zeros(n_clus,1); % minoraxis at 5% max via kernel density estimation
properties.vbSPT.fastminAx50_removeDiffuseIn = zeros(n_clus,1); % minoraxis at 50% max via kernel density estimation
properties.vbSPT.fastminAx95_removeDiffuseIn = zeros(n_clus,1); % minoraxis at 5% max via kernel density estimation
properties.vbSPT.slowbound50_removeDiffuseIn = cell(n_clus,1); % boundary at 50% max via kernel density estimation
properties.vbSPT.slowbound95_removeDiffuseIn = cell(n_clus,1); % boundary at 5% max via kernel density estimation
properties.vbSPT.fastbound50_removeDiffuseIn = cell(n_clus,1); % boundary at 50% max via kernel density estimation
properties.vbSPT.fastbound95_removeDiffuseIn = cell(n_clus,1); % boundary at 5% max via kernel density estimation
properties.vbSPT.slowCOM50_removeDiffuseIn = zeros(n_clus,2); % mask center of mass at 50% max via kernel density estimation
properties.vbSPT.slowCOM95_removeDiffuseIn = zeros(n_clus,2); % mask center of mass at 5% max via kernel density estimation
properties.vbSPT.fastCOM50_removeDiffuseIn = zeros(n_clus,2); % mask center of mass at 50% max via kernel density estimation
properties.vbSPT.fastCOM95_removeDiffuseIn = zeros(n_clus,2); % mask center of mass at 5% max via kernel density estimation
...
for i=1:n_clus
   properties.trackIDs{i} = clusterTrackIDs{i};
   properties.N(i) = numel(clusterTrackIDs{i});
   clus_track_centroids = centroids(clusterTrackIDs{i},:)*0.1067;
   allLocalizations = cell2mat(cellfun(@(x) x(:,1:2),expdata.smLinked(clusterTrackIDs{i}),'uniformoutput',false))*0.1067; % fixed this May 11, need to re-do
   ...
   % Remove tracks that diffuse in
   allStarts = cell2mat(cellfun(@(x) x(1,1:2),expdata.smLinked(clusterTrackIDs{i}),'uniformoutput',false))*0.1067; % fixed this May 11, need to re-do
   cluster_region = polyshape(bound50);
   diffusedIn = ~isinterior(cluster_region,allStarts);
   allTracks = cellfun(@(x) x(1,1:2)*0.1067,expdata.smLinked(clusterTrackIDs{i}),'uniformoutput',false); % fixed this May 11, need to re-do
   trackSubset = cell2mat(allTracks(~diffusedIn));
   [area50_sub,r50_sub,circ50_sub,majAx50_sub,minAx50_sub,eccen50_sub,bound50_sub,com50_sub,...
    area95_sub,r95_sub,circ95_sub,majAx95_sub,minAx95_sub,eccen95_sub,bound95_sub,com95_sub] = fit_KDE_radius(trackSubset);
   properties.allLoc.KDEeffR50_removeDiffuseIn(i) = r50_sub;
   properties.allLoc.KDEeffR95_removeDiffuseIn(i) = r95_sub;
   properties.allLoc.KDEcirc50_removeDiffuseIn(i) = circ50_sub;
   properties.allLoc.KDEcirc95_removeDiffuseIn(i) = circ95_sub;
   properties.allLoc.KDEeccen50_removeDiffuseIn(i) = eccen50_sub;
   properties.allLoc.KDEeccen95_removeDiffuseIn(i) = eccen95_sub;
   properties.allLoc.KDEarea50_removeDiffuseIn(i) = area50_sub;
   properties.allLoc.KDEarea95_removeDiffuseIn(i) = area95_sub;
   properties.allLoc.KDEmajAx50_removeDiffuseIn(i) = majAx50_sub;
   properties.allLoc.KDEmajAx95_removeDiffuseIn(i) = majAx95_sub;
   properties.allLoc.KDEminAx50_removeDiffuseIn(i) = minAx50_sub;
   properties.allLoc.KDEminAx95_removeDiffuseIn(i) = minAx95_sub;
   properties.allLoc.KDEbound50_removeDiffuseIn{i} = bound50_sub;
   properties.allLoc.KDEbound95_removeDiffuseIn{i} = bound95_sub;
   properties.allLoc.KDEmaskCOM50_removeDiffuseIn(i,:) = com50_sub;
   properties.allLoc.KDEmaskCOM95_removeDiffuseIn(i,:) = com95_sub;
   ...
   all_slow_localizations = cell2mat(cellfun(@(x,y) x(y==1,1:2)*0.1067,expdata.smLinked(properties.trackIDs{i}),expdata.vbspt(properties.trackIDs{i}),'uniformoutput',false));
   all_fast_localizations = cell2mat(cellfun(@(x,y) x(y==2,1:2)*0.1067,expdata.smLinked(properties.trackIDs{i}),expdata.vbspt(properties.trackIDs{i}),'uniformoutput',false));
   ...
   % Diffuse-in analysis, slow/fast sizes
   % Obtain track IDs, for the cluster, after removing those that diffsed in.
   allStartIDs = cellfun(@(x) x(1,16),expdata.smLinked(clusterTrackIDs{i}));
   IDsubset = allStartIDs(~diffusedIn);   
   % Use IDsubset for getting slow/fast localizations
   all_slow_localizations_removeDiffuseIn = cell2mat(cellfun(@(x,y) x(y==1,1:2)*0.1067,expdata.smLinked(IDsubset),expdata.vbspt(IDsubset),'uniformoutput',false));
   all_fast_localizations_removeDiffuseIn = cell2mat(cellfun(@(x,y) x(y==2,1:2)*0.1067,expdata.smLinked(IDsubset),expdata.vbspt(IDsubset),'uniformoutput',false));


   % kernel density estimate estimates of slow and fast zone sizes
   [area50,r50,circ50,majAx50,minAx50,eccen50,bound50,com50,...
    area95,r95,circ95,majAx95,minAx95,eccen95,bound95,com95] = fit_KDE_radius(all_slow_localizations_removeDiffuseIn);
   properties.vbSPT.sloweffR50_removeDiffuseIn(i) = r50;
   properties.vbSPT.sloweffR95_removeDiffuseIn(i) = r95;
   properties.vbSPT.slowcirc50_removeDiffuseIn(i) = circ50;
   properties.vbSPT.slowcirc95_removeDiffuseIn(i) = circ95;
   properties.vbSPT.sloweccen50_removeDiffuseIn(i) = eccen50;
   properties.vbSPT.sloweccen95_removeDiffuseIn(i) = eccen95;
   properties.vbSPT.slowarea50_removeDiffuseIn(i) = area50;
   properties.vbSPT.slowarea95_removeDiffuseIn(i) = area95;
   properties.vbSPT.slowmajAx50_removeDiffuseIn(i) = majAx50;
   properties.vbSPT.slowmajAx95_removeDiffuseIn(i) = majAx95;
   properties.vbSPT.slowminAx50_removeDiffuseIn(i) = minAx50;
   properties.vbSPT.slowminAx95_removeDiffuseIn(i) = minAx95;
   properties.vbSPT.slowbound50_removeDiffuseIn{i} = bound50;
   properties.vbSPT.slowbound95_removeDiffuseIn{i} = bound95;
   properties.vbSPT.slowCOM50_removeDiffuseIn(i,:) = com50;
   properties.vbSPT.slowCOM95_removeDiffuseIn(i,:) = com95;

   [area50,r50,circ50,majAx50,minAx50,eccen50,bound50,com50,...
    area95,r95,circ95,majAx95,minAx95,eccen95,bound95,com95] = fit_KDE_radius(all_fast_localizations_removeDiffuseIn);
   properties.vbSPT.fasteffR50_removeDiffuseIn(i) = r50;
   properties.vbSPT.fasteffR95_removeDiffuseIn(i) = r95;
   properties.vbSPT.fastcirc50_removeDiffuseIn(i) = circ50;
   properties.vbSPT.fastcirc95_removeDiffuseIn(i) = circ95;
   properties.vbSPT.fasteccen50_removeDiffuseIn(i) = eccen50;
   properties.vbSPT.fasteccen95_removeDiffuseIn(i) = eccen95;
   properties.vbSPT.fastarea50_removeDiffuseIn(i) = area50;
   properties.vbSPT.fastarea95_removeDiffuseIn(i) = area95;
   properties.vbSPT.fastmajAx50_removeDiffuseIn(i) = majAx50;
   properties.vbSPT.fastmajAx95_removeDiffuseIn(i) = majAx95;
   properties.vbSPT.fastminAx50_removeDiffuseIn(i) = minAx50;
   properties.vbSPT.fastminAx95_removeDiffuseIn(i) = minAx95;
   properties.vbSPT.fastbound50_removeDiffuseIn{i} = bound50;
   properties.vbSPT.fastbound95_removeDiffuseIn{i} = bound95;
   properties.vbSPT.fastCOM50_removeDiffuseIn(i,:) = com50;
   properties.vbSPT.fastCOM95_removeDiffuseIn(i,:) = com95;

   
   
   % lifetimes and recruitment intervals
   track_starts = cellfun(@(x) x(  1,end),expdata.smLinked(clusterTrackIDs{i}));
   track_ends   = cellfun(@(x) x(end,end),expdata.smLinked(clusterTrackIDs{i}));
